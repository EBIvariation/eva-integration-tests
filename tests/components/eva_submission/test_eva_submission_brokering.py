...
class TestEvaSubmissionValidation(TestWithDockerCompose):
    ...
    def setUp(self):
        ...
        self.eload_number = random.randint(1, 10000)
    ...
    def test_submission_with_new_metadata_spreadsheet(self):
        ...
        validation_cmd = (
            f"docker exec {self.container_name} sh -c 'validate_submission.py --eload {self.eload_number} > {self.container_eload_dir}/ELOAD_{self.eload_number}/validation.out 2>&1'"
        )
        # Run validation from command line
        run_quiet_command("run eva_submission validate_submission script", validation_cmd)

        brokering_cmd = (
            f"docker exec {self.container_name} sh -c 'broker_submission.py --eload {self.eload_number} > {self.container_eload_dir}/ELOAD_{self.eload_number}/broker.out 2>&1'"
        )
        ...
        copy_files_from_container(self.container_name,
                                  ...
                                  self.test_run_dir)
        # assert results
        self.assert_brokering_pass_in_config(os.path.join(self.test_run_dir, f'ELOAD_{self.eload_number}', f'.ELOAD_{self.eload_number}_config.yml'))
